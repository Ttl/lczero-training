...
class TFProcess:
    ...
    def replace_weights_v2(self, proto_filename):
        ...
        for weight in self.model.weights:
            try:
                ...
                print('No values for tensor {} in protobuf'.format(weight.name))
    ...
    def save_leelaz_weights_v2_(self, filename):
        all_tensors = []
        all_weights = []
    ...
    def batch_norm_v2(self, input, name, scale=False):
        ...
        if self.renorm_enabled:
            ...
            return tf.keras.layers.BatchNormalization(
                epsilon=1e-5, axis=axis, center=True,
                scale=scale, name=name)(input)
    ...
    def squeeze_excitation_v2(self, inputs, channels, name):
        ...
        squeezed = tf.keras.layers.Activation('relu')(tf.keras.layers.Dense(channels // self.SE_ratio, kernel_regularizer=self.l2reg, kernel_initializer='glorot_normal',
            name=name + '/se/dense1')(pooled))
        excited = tf.keras.layers.Dense(2 * channels, kernel_regularizer=self.l2reg, kernel_initializer='glorot_normal', name=name + '/se/dense2')(squeezed)
    ...
    def conv_block_v2(self, inputs, filter_size, output_channels, name, bn_scale=False):
        conv = tf.keras.layers.Conv2D(output_channels, filter_size, use_bias=False, padding='same', kernel_initializer=tf.keras.initializers.he_normal(), kernel_regularizer=self.l2reg, data_format=self.data_format, name=name + '/conv2d')(inputs)
    ...
    def residual_block_v2(self, inputs, channels, name):
        conv1 = tf.keras.layers.Conv2D(channels, 3, use_bias=False, padding='same', kernel_initializer=tf.keras.initializers.he_normal(), kernel_regularizer=self.l2reg, data_format=self.data_format, name=name + '/1/conv2d')(inputs)
        out1 = tf.keras.layers.Activation('relu')(self.batch_norm_v2(conv1, name + '/1/bn', scale=False))
        conv2 = tf.keras.layers.Conv2D(channels, 3, use_bias=False, padding='same', kernel_initializer=tf.keras.initializers.he_normal(), kernel_regularizer=self.l2reg, data_format=self.data_format, name=name + '/2/conv2d')(out1)
        out2 = self.squeeze_excitation_v2(self.batch_norm_v2(conv2, name + '/2/bn', scale=True), channels, name)
    ...
    def construct_net_v2(self, inputs):
        ...
        if self.POLICY_HEAD == pb.NetworkFormat.POLICY_CONVOLUTION:
            ...
            conv_pol2 = tf.keras.layers.Conv2D(80, 3, use_bias=True, padding='same', kernel_initializer=tf.keras.initializers.he_normal(), kernel_regularizer=self.l2reg, data_format=self.data_format, name='policy')(conv_pol)
            if self.data_format == 'channels_last':
