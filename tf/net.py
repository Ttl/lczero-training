...
class Net:
    def __init__(self,
            net=pb.NetworkFormat.NETWORK_SE,
        ...
        self.pb.format.network_format.output = output
    ...
    def set_policyformat(self, policy):
        if policy == pb.NetworkFormat.POLICY_CONVOLUTION:
            self.pb.min_version.major = 0
            self.pb.min_version.minor = 21
        self.pb.format.network_format.policy = policy
    ...
    def parse_txt(self, filename):
        ...
        if version == 3:
            self.set_networkformat(pb.NetworkFormat.NETWORK_SE)
